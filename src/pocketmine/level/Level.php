...
class Level implements ChunkManager, Metadatable
{
	...
	public function unload(bool $force = false): bool
	{
		...
		foreach ($this->getPlayers() as $player) {
			...
			} else if ($defaultLevel instanceof Level) {
	...
	*/
	...
	public function getBlock(Vector3 $pos, $cached = true): Block
	{
		...
		} else if ($pos->y >= 0 and $pos->y < 128 and isset($this->chunks[$chunkIndex = Level::chunkHash($pos->x >> 4, $pos->z >> 4)])) {
	...
	public function registerChunkLoader(ChunkLoader $loader, int $chunkX, int $chunkZ, bool $autoLoad = true)
	{
		...
		} else if (isset($this->chunkLoaders[$index][$hash])) {
			return;
		}
		...
		if ($optimizeRebuilds) {
			...
			foreach ($blocks as $b) {
				...
				if ($b instanceof Block) {
					...
					$fullBlock = $this->getFullBlock($b->getFloorX(), $b->getFloorY(), $b->getFloorZ());
					$pk->x = $b->x;
					$pk->z = $b->z;
					$pk->y = $b->y;
					$pk->blockId = $fullBlock >> 4;
	...
	/** @return Block */
	public function getBlockFromCache(int $index)
	{
		return $this->blockCache[$index];
	}
	...
	public function getBlockFromXYZ(float|int $x, float|int $y, float|int $z, $cached = true)
	{
		$x = (int) floor($x);
		$y = (int) floor($y);
		$z = (int) floor($z);
		...
		if ($cached and isset($this->blockCache[$index])) {
			return $this->blockCache[$index];
		} else if ($y >= 0 and $y < 128 and isset($this->chunks[$chunkIndex = Level::chunkHash($x >> 4, $z >> 4)])) {
	...
	public function useBreakOn(Vector3 $vector, Item &$item = null, Player $player = null, bool $createParticles = false): bool
	{
		...
		if ($player !== null) {
			...
			if ($player->isSurvival() and $item instanceof Item and !$target->isBreakable($item)) {
				$ev->setCancelled();
			} else if (!$player->isOp() and ($distance = $this->server->getSpawnRadius()) > -1) {
		...
		} else if ($item !== null and !$target->isBreakable($item)) {
			return false;
		} else {
	...
	public function useItemOn(Vector3 $vector, Item &$item, int $face, float $fx = 0.0, float $fy = 0.0, float $fz = 0.0, Player $player = null): bool
	{
		...
		} else if ($target->canBeActivated() === true and $target->onActivate($item, $player) === true) {
			return true;
		}
	...
	public function generateChunkCallback(int $x, int $z, FullChunk $chunk)
	{
		...
		} else if (isset($this->chunkGenerationQueue[$index]) or isset($this->chunkPopulationLock[$index])) {
